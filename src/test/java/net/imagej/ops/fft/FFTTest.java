...
public class FFTTest extends AbstractOpBenchmark {
	...
	public void testFastFft3dOp() {
		...
		for (int i = 115; i < 135; i++) {
			...
			// Perform an fft. By default the smallest possible size is used
			Img<ComplexFloatType> fft1 =
				(Img<ComplexFloatType>) ops.run("fft", inOriginal);
			...
			// create an image to be used for the inverse, using the original size
			Img<FloatType> inverseOriginalSmall =
				(Img<FloatType>) ops
					.run(CreateImgDefault.class, new ArrayImgFactory<FloatType>(),
						new FloatType(), originalDimensions);

			// create an inverse image to be used for the inverse, using the original
			// size
			...
			// create an inverse image to be used for the inverse, using the fast size
			...
			// invert the "fast" fft. The inverse will should be the original size.
			ops.run("ifft", inverseOriginalFast, fft2);

			// invert the "fast" fft that was acheived by explicitly using an image
			// that had "fast" dimensions. The inverse will be the fast size this
			// time.
